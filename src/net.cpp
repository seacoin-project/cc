...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            addrConnect = CService("37.247.55.42", 80); // ip1.crowncoin.org

            if (nLookup == 1)
            {
                CService addrIP("ip1.crowncoin.org", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }

            pszGet = "GET / HTTP/1.1\r\n"
                     "Host: ip1.crowncoin.org\r\n"
        ...
        {
            addrConnect = CService("37.247.55.42", 80); // ip2.crowncoin.org

            if (nLookup == 1)
            {
                CService addrIP("ip2.crowncoin.org", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }

            pszGet = "GET /simple/ HTTP/1.1\r\n"
                     "Host: ip2.crowncoin.org\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";
...
static const char *strMainNetDNSSeed[][2] = {
	{"crowncoin.org", "seed.crowncoin.org"},
	{"crowncoin.com", "seed.crowncoin.com"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Crowncoin is probably already running."), addrBind.ToString().c_str());
        else
