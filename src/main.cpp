...
// Copyright (c) 2014 Crowncoin developers http://fuguecoin.org
...
uint256 hashGenesisBlock("0x000000c336e475adb4de639c9d98c0a01111700075a327501da6045e3455cde3");
static const unsigned int timeGenesisBlock = 1395485888;
...
const string strMessageMagic = "Crowncoin Signed Message:\n";
...
static const int64 nBlockRewardStartCoin = 5000 * COIN;

static const int64 nTargetTimespan = 30; // 30s
static const int64 nTargetSpacing = 30; // 30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 LimitUp = nTargetTimespan * 100 / 115;
    int64 LimitUp2 = nTargetTimespan * 100 / 100; // keep diff

    if (nActualTimespan < nTargetTimespan/4 && pHeight >= 30)
       {
        nActualTimespan = LimitUp2;
       }
    if (nActualTimespan < LimitUp && pHeight <= 29)
       {
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "SFO tech aims to meet runway safety standards";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xfa6c4d507ded70a48bca30855b8281a1d4efc49d0ff315e0e2b9ba7725155024"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xd9 };
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = Hashfugue(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = Hashfugue(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = Hashfugue(vRecv.begin(), vRecv.begin() + nMessageSize);
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running CrowncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("CrowncoinMiner terminated\n");
        throw;
    }
}
