...
const QString BITCOIN_IPC_PREFIX("crowncoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click crowncoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start crowncoin: click-to-pay handler");
    else
...
{
    // clicking on crowncoin: URLs creates FileOpen events on the Mac:
