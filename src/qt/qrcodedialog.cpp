...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CC")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CC unit input in lnReqAmount, we generate the URI with CC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
